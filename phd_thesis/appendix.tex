...
\begin{proof}
  ...
  To establish the third inequality observe first that $-(\tilde y^t_{k})^T{d}_k = -(y^t_{k})^T{d}_k - (\tilde y^t_k-y^t_k)^Td_k\leq -(y^t_{k})^T{d}_k + \norm{\tilde y^t_k-y^t_k}_* \norm{d_k}\leq -(y^t_{k})^T{d}_k + \norm{d_k}/t$. \reviewChanges{Second, remark that we have 
    \begin{align*}
      ...
      \leq & \max_{s\in \supp} s^T(h^t_{k}+(\tilde y^t_k-y_k^t)) \\
      \leq & \max_{s\in \supp} s^Th^t_{k}+\max_{s\in \supp} s^T(\tilde y^t_k-y_k^t)\leq \norm{s}\norm{\tilde y^t_k-y_k^t}_* \leq 1/t \max_{s\in \supp} \norm{s} .
    \end{align*}
...
Byassumption, $g$ is concave in the uncertain parameter, so by Jensen's inequality,
\begin{align*}
...
&= -h_{jt} - ((-h_{jt})^{t/(t+1)}(F_{jt}z_j)^{1/(t+1)})(t^{1/(t+1)} + t^{-1/(t+1)}),
	\end{align*}
after combining terms. Note that $-(-h_{jt})^{t/(t+1)}(F_{jt}z_j)^{1/(t+1)}$ can be replaced by an auxiliary variable $\delta_{jtk} \leq 0$, by introducing the power cone constraint $(-h_{jt})^{t/(t+1)}(F_{jt}z_j)^{1/(t+1)} \geq |\delta_{jtk} |$. Bysubstituting these results into~\eqref{eq:robustopt_p} and further vectoring some constraints, we arrive at the desired formulation.
...
Byassumption, $\epsnom_t\sim (\log(\beta_t^{-1})/t)^{1/m}$, so their averages are $O((\log(\beta_T^{-1})/T)^{1/m}).$
Note that by the summability of $\beta_t$, the convergence rate of $\beta_t$ is at least sublinear. 
Next, recall by the triangle inequality,  $$d^K_{t,1} \leq W_1(\hat{\prob}_t,\prob) + W_1(\prob,\prob^K_\star) + W_1(\prob^K_\star,\prob^K_{t}).$$ ByTheorem~\ref{thm:measure_gen}, we can construct a summable sequence $\beta_t$ such that $\prob(W_1(\hat{\prob}_t,\prob) \leq \epsnom_t) \geq  1-\beta_t$, where $\epsnom_t$ is the same as above. ByTheorem~\ref{lem:conditional_Conv}, a similar result holds for $W_1(\prob^K_\star,\hat{\prob}^K_t)$, with a sequence of radii $\tilde{\epsnom}_t\leq O(\epsnom_t)$ for $t \geq \tau$. Since $T \gg \tau$, the finite values $\tilde{\epsnom}_t/T$ for $t\leq \tau$ are dominated by $O(\epsnom_T)$.
...
Bydefinition of the Wasserstein metric,
\begin{equation*}
\begin{aligned}
...
Bythe finite sample performance guarantee in Theorem~\ref{lemma:guarantees_fin} and the summability of $\beta_t$, the Borel–Cantelli Lemma implies that $$ \mathbf{P}^\infty\left\{ H_\star \leq \limsup_{t \rightarrow \infty } \Expect_\prob[f(\unc,z^K_{t})]\leq  \limsup_{t \rightarrow \infty } H^K_t\right\} = 1.$$
...
\RC{$$ \uncset(\theta,x) = \left\{u = A(x)v+b(x) \mid \| v\|_p \leq 1,\quad u \in \supp\right\},$$
...
, and $\supp$ includes additional support information such as upper and lower bounds. We let $p$ be the order of the norm, which is any integer value $\geq 1$, or equal to $\infty$. In the latter case, this becomes the box uncertainty set. The trainable parameter \RC{$\theta = (A(\cdot),b(\cdot))$, the mapping from the context parameter to the reshaping matrices.} Note that if $A = \rho I$, $b = 0$, and~$\supp = \reals^m$, this is equivalent to the traditional ellipsoidal uncertainty set ${ \{ u \mid\| u\|_p \leq \rho \}}$. 
For the maximum of concave constraint, we have the following reformulation,
% The proofs are in Appendix~\ref{apx:boxproof}. 
\begin{equation*}
	\begin{array}{ll}
	\text{minimize} & f(z)\\
	\mbox{subject to} & [-g_l]^\star(z,h_l)+ \sigma_\supp(\omega_l)  -b\RC{(x)}^T(h_l + \omega_l) + \|A\RC{(x)}^T(h_l + \omega_l) \|_q \leq 0, \quad l = 1,\dots,L,
	\end{array}
	\end{equation*}
...
\begin{equation*}
	...
	\begin{array}{ll}
		\mbox{minimize} & f(z)\\
		\mbox{subject to} & a_l(z) + \sigma_\supp(\omega_l)+ b\RC{(x)}^T(P_l(z) - \omega_l) +  \|A\RC{(x)}^T(\omega_l -P_l(z))\|_q \le 0, \quad l = 1,\dots,L.	\end{array}
\end{equation*}

\paragraph{Budget uncertainty.}
The budget uncertainty set requires both box and ellipsoidal constraints,
\RC{$$ \uncset_{\text{bud}}(\theta,x) = \{u = A\RC{(x)}v+b\RC{(x)} \mid \| v\|_\infty \leq \rho_1, \quad \| v \|_1 \leq \rho_2, \quad u \in \supp\},$$
...
\begin{equation*}
	\begin{array}{ll}
		{\text{minimize}} & f(z) \\
		\mbox{subject to} & [-g_l]^\star(z,h_l)  + \sigma_\supp(\omega_l) -  b\RC{(x)}^T(h_l + \omega_l) + \rho_1\|r_l - A\RC{(x)}^T(h_l + \omega_l)\|_1  + \rho_2\|r_l\|_\infty \leq 0,\\
...
\begin{equation}
	...
	\begin{array}{ll}
		\mbox{minimize} & f(z)\\
		\mbox{subject to} &  a_l(z) + \sigma_\supp(\omega_l) + b\RC{(x)}^T(P_l(z) - \omega_l) + \rho_1\|r_l - A\RC{(x)}^T(P_l(z) +\omega_l)\|_1 + \rho_2\|r_l\|_\infty \leq 0,\\
...
\RC{\begin{equation*}
    \uncset(\theta) = \{ u = A\RC{(x)}v+b\RC{(x)} \mid Dv \leq d, \quad u \in \supp\},\end{equation*}
...
\begin{array}{ll}
	{\text{minimize}} & f(z)\\
	\mbox{subject to} & [-g_l]^\star(z,\omega_l) - b\RC{(x)}^T(\omega_l + \gamma_l) + \sigma_\supp(\gamma_l) \leq 0, \quad l = 1,\dots,L\\
...
\begin{equation*}
	...
	\mbox{subject to} &\lambda \rho^p + \sum_{k=1}^K w_k s_k \leq 0\\
	 & [-g_l]^\star(x,h_{lk}) -b\RC{(x)}^T(h_{lk} + \omega_{lk}) + \sigma_\supp(\omega_{lk}) - (A\RC{(x)}^T(h_{lk} + \omega_{lk}))^T \bar{d}_k \\
...
\begin{equation}
	...
	\begin{aligned}
		\begin{array}{ll}
			...
			& a_l(z) + \sigma_\supp(\omega_{lk}) - b\RC{(x)}^T(\omega_{lk} - P_l(x)) - (A\RC{(x)}^T(\omega_{lk} - P_l(z)))^T\bar{d}_k \\
...
At a fixed $\theta$ and for any context $x_i$, the inner level problem~\eqref{eq:robust_prob} with respect to the $1$-norm uncertainty set with a polyhedral support $\supp=\{u \in \reals^m~|~Cu \leq d \}$, that is, 
\begin{equation*}
\label{unc_set}
    \uncset(\theta,x) = \left\{u = A(x)v+b(x) \mid \|v\|_1 \leq \rho ,~ u \in \supp\right\},
\end{equation*} 
...
% 		&\mbox{subject to} & (1/2)\sum_{i=1}^n (h_i^TP_i^{-1}h_i)/x_i - b^Th_i + \sigma_\supp(\omega) - b^T\omega + \|\sum_{i=1}^n A^Th_i + A^T\omega\|_q \le 0\\		&& x \geq 0,
% 	\end{array}
% \end{equation}
...
& \begin{array}[t]{ll}
	\underset{Av+b\in \supp}{\sup} & g(z,Av+b) \\
	\mbox{subject to} & \|v\|_p \le 1
\end{array}\\
&= \begin{array}[t]{ll}
	\underset{Av+b\in \supp}{\sup}  ~\underset{\lambda \geq 0}{\inf}~g(z,Av+b)  - \lambda\| v\|_p  + \lambda\\
	\end{array}\\
	&= \begin{array}[t]{ll}
	\underset{Av+b\in \supp}{\sup} ~ \underset{\lambda \geq 0}{\inf}  ~\underset{l}{\max}~g_l(z,Av+b)  - \lambda\|v \|_p  + \lambda\\
		\end{array}\\
		&= \begin{array}[t]{ll}
			\underset{l}{\max}~ \underset{Av+b\in \supp}{\sup} ~ \underset{\lambda \geq 0}{\inf}  ~g_l(z,Av+b)  - \lambda\| v\|_p  + \lambda\\
				\end{array}\\
			&= \begin{array}[t]{ll}
					\underset{l}{\max}~ \underset{\lambda \geq 0}{\inf} ~ \underset{Av+b\in \supp}{\sup}~ g_l(z,Av+b)  - \lambda\| v \|_p  + \lambda\\
						\end{array}\\
&= \begin{array}[t]{ll}
	\underset{\lambda_l \geq 0, \tau}{\inf} & \tau\\
	\mbox{subject to} & \lambda_l + \underset{Av+b\in \supp}{\sup } g_l(z,Av+b) - \lambda_l  \|v \|_p \leq \tau, \quad l = 1,\dots,L,
\end{array}
\end{align*}}
...
\begin{equation*}
\begin{aligned}
		\lambda_l + [-g_l(z,A\cdot+b) +\chi_\supp(A\cdot+b) + \lambda_l c]^\star(0).\\
\end{aligned}
\end{equation*}
...
\begin{align*}
	[-g_l(z,A\cdot+b)   +\chi_\supp(A\cdot+b)+ \lambda_l c]^\star (0) =  \underset{\omega_l, h_l, y_l}{\inf} ([-g_l]^\star(z,h_l) &- b^T(h_l + \omega_l) + \sigma_\supp(\omega_l)+\indicator_{\|y_l\|_q \leq \lambda_l}\\
...
\mbox{subject to} & [-g_l]^\star(z,h_l)+ \sigma_\supp(\omega_l)  -b^T(h_l + \omega_l) + \|y_l\|_q \leq \tau, \quad l = 1,\dots,L \\
...
\begin{equation*}
	...
	\mbox{subject to} & [-g_l]^\star(z,h_l)+ \sigma_\supp(\omega_l)  -b^T(h_l + \omega_l) + \|A^T(h_l + \omega_l) \|_q \leq 0, \quad l = 1,\dots,L.	\end{array}
	\end{aligned}
	\end{equation*}
...
\begin{equation*}
	\begin{aligned}
& \begin{array}[t]{ll}
	\underset{Az+b\in \supp}{\sup} & g(z,Av+b) \\
	\mbox{subject to} & \| v \|_\infty \le \rho_1\\
	 & \| v \|_1 \le \rho_2\\
\end{array}\\
&= \begin{array}[t]{ll}
	\underset{Av+b\in \supp}{\sup}~ \underset{\lambda \geq 0}{\inf} ~\underset{l}{\max} ~ g_l(z,Av+b)  - \lambda_1\| v \|_\infty  + \lambda_1\rho_1 - \lambda_2\| v\|_1  + \lambda_2\rho_2\\
	\end{array}\\
 &=  \begin{array}[t]{ll}
	\underset{l}{\max} ~\underset{Av+b\in \supp}{\sup} ~ \underset{\lambda \geq 0}{\inf} ~g_l(z,Av+b)  - \lambda_1\| z \|_\infty  + \lambda_1\rho_1 - \lambda_2\| v\|_1  + \lambda_2\rho_2\\
	\end{array}\\
	&= \begin{array}[t]{ll}
		\underset{l}{\max}~\underset{\lambda \geq 0}{\inf}~\underset{Av+b\in \supp}{\sup} ~g_l(z,v)  - \lambda_1\| v\|_\infty  + \lambda_1\rho_1 - \lambda_2\| v \|_1  + \lambda_2\rho_2,\\
		\end{array}\\
\end{aligned}
\end{equation*}
...
\begin{equation*}
	...
	\begin{array}[t]{ll}
		...
		\mbox{subject to} & \lambda_{l1}\rho_1 + \lambda_{l2}\rho_2 + \underset{Av+b\in \supp}{\sup } g_l(z,Av+b) -\lambda_{l1}\| v \|_\infty - \lambda_{l2}\| v \|_1 \leq \tau, \quad l = 1,\dots,L.
	\end{array}
	\end{aligned}
	\end{equation*}
...
\begin{equation*}
\begin{aligned}
		& \lambda_{l1}\rho_1 +  \lambda_{l2}\rho_2 +  [-g_l(z,A\cdot+b) +\chi_\supp(A\cdot+b) + \lambda_{l1} c_1 + \lambda_{l2} c_2]^\star(0)\\
		&= 	\underset{h_l,\omega_l, y_l}{\inf} \lambda_{l1}\rho_1 + \lambda_{l2}\rho_2 + [-g_l]^\star(z,h_l) - b^T(h_l + \omega_l) + \sigma_\supp(\omega_l)  +  [\lambda_{l1} c_1 + \lambda_{l2} c_2 ]^\star(-y_l) \\
  &\hspace{12cm}+ \indicator_{A^T(h_l + \omega_l) = y_l} \\
		&= \underset{h_l,\omega_l, y_l, r_l}{\inf} \lambda_{l1}\rho_1 + \lambda_{l2}\rho_2 + [-g_l]^\star(z,h_l) - b^T(h_l + \omega_l)+ \sigma_\supp(\omega_l)  + \indicator_{\|r_l-y_l\|_1 \leq \lambda_{l1}} + \indicator_{\|r_l\|_\infty \leq \lambda_{l2}} \\
  & \hspace{12cm} + \indicator_{A^T(h_l + \omega_l) = y_l}
\end{aligned}
\end{equation*}
...
\begin{equation*}
	...
	\begin{array}[t]{ll}
		...
		\mbox{subject to} & [-g_l]^\star(z,h_l)  + \sigma_\supp(\omega_l) -  b^T(h_l + \omega_l) + \rho_1\|r_l - y_l\|_1  + \rho_2\|r_l\|_\infty \leq \tau, \\
  &\hfill k =  1, \dots,K \\
	 & A^T(h_l + \omega_l) = y_l,  \quad l =  1, \dots,L.
	\end{array}
	\end{aligned}
...
\begin{equation*}
	\begin{array}[t]{ll}
		{\text{minimize}} & f(z) \\
		\mbox{subject to} & [-g_l]^\star(z,h_l)  + \sigma_\supp(\omega_l) -  b^T(h_l + \omega_l) + \rho_1\|r_l - A^T(h_l + \omega_l)\|_1  + \rho_2\|r_l\|_\infty \leq 0,\\
...
\end{array}\\
&= \begin{array}[t]{ll}
	\underset{Av+b\in \supp}{\sup} ~\underset{h \geq 0}{\inf} ~g(z,Av+b)  - (Dv)^Th   + d^Th\\
	\end{array}\\
	&= \begin{array}[t]{ll}
		\underset{Av+b\in \supp}{\sup}~\underset{h \geq 0}{\inf}~\underset{l}{\max}~g_l(z,Av+b)  - (Dv)^Th   + d^Th\\
		\end{array}\\
		&= \begin{array}[t]{ll}
			\underset{l}{\max}~\underset{Av+b\in \supp}{\sup}~\underset{h \geq 0}{\inf}~g_l(z,Av+b)  - (Dv)^Th   + d^Th\\
			\end{array}\\
		&= \begin{array}[t]{ll}
				\underset{l}{\max}~\underset{h \geq 0}{\inf}~\underset{Av+b\in \supp}{\sup}~g_l(z,Av+b)  - (Dv)^Th   + d^Th\\
				\end{array}\\
&= \begin{array}[t]{ll}
	\underset{h\geq 0,\tau}{\inf} & \tau \\
	\mbox{subject to}& d^Th_l + \underset{Av+b\in \supp}{\sup } g_l(z,Av+b) - (Dv)^Th_l \leq \tau, \quad l = 1,\dots,L
\end{array}\\
&= \begin{array}[t]{ll}
	\underset{h \geq 0, \omega,\tau, y, \gamma}{\inf} &\tau\\
	\mbox{subject to}& d^Th_l + [-g_l]^\star(z,\omega_l) - b^T(\omega_l + \gamma_l) + \sigma_\supp(\gamma_l) + \indicator_{D^Th_l = -y_l} \\
 &\hfill  + \indicator_{A^T(\omega_l+ \gamma_l) = y_l} \leq \tau, \quad l = 1,\dots,L
\end{array}\\
&= \begin{array}[t]{ll}
	\underset{h \geq 0,\omega,\tau,\gamma}{\inf} &\tau\\
	\mbox{subject to}& [-g_l]^\star(z,\omega_l) - b^T(\omega_l + \gamma_l) + \sigma_\supp(\gamma_l) \leq \tau, \quad l = 1,\dots,L\\
...
\begin{array}[t]{ll}
	{\text{minimize}} & f(z)\\
	\mbox{subject to} & [-g_l]^\star(z,\omega_l) - b^T(\omega_l + \gamma_l) + \sigma_\supp(\gamma_l) \leq 0, \quad l = 1,\dots,L\\
	 &A^T(\omega_l+ \gamma_l) = -D^Th_l,\quad l = 1,\dots,L.
\end{array}
\end{equation*}
...
\begin{equation*}
  ...
  &+ \indicator_{A^T(h_{lk} + \omega_{lk})= y_{lk}} + \sigma_\supp(\omega_{lk})+[\lambda c_k]^\star (-y_{lk}))
	\end{aligned}
\end{equation*}
and
\begin{equation*}
...
\mbox{s.t.} & [-g_l]^\star(z,h_{lk}) -b^T(h_{lk} + \omega_{lk}) + \sigma_\supp(\omega_{lk}) - (A^T(h_{lk} + \omega_{lk}))^T \bar{d}_k \\
...
\begin{equation*}
	...
	\mbox{subject to} &\lambda \epsilon^p + \sum_k^K w_k s_k \leq 0\\
	 & [-g_l]^\star(z,h_{lk}) -b^T(h_{lk} + \omega_{lk}) + \sigma_\supp(\omega_{lk}) - (A^T(h_{lk} + \omega_{lk}))^T \bar{d}_k \\
& \hfill + \phi(q)\lambda \left\|A^T(h_{lk} + \omega_{lk})/\lambda \right\|^q_* \le s_k,\\
& \hfill l = 1,\dots,L, \quad k = 1,\dots,K.
	\end{array}
	\end{aligned}
...
% \section{Uncertainty sets and general reformulations}
% \label{appx:gen_sets}
% We derive reformulations of~\eqref{eq:canon_robust} for the uncertainty sets not given in Section~\ref{sec:uncertainty_sets}.
% % Here, we consider the case for a single $g$, $\ie$, $I_1 = 1$.
% % For multiple uncertain constraints, the same reformulation techniques can be applied to each constraint.
% The dimension of $A$, unless otherwise specified, is $m\times \hat{m}$, where $\hat{m} \leq m$, and the dimension of $b$ is $m$.
% % Reformulations for linear constraints are in Appendix~\ref{sec:RO_reform_linear}. 
% % We suppress the dependency of $g$ on $y$ here and for all following sections on reformulations.
% % We consider two general types of functions $g$: the sum of concave functions,
% % $$g(x,u) = \sum_{j=1}^J g_j(u,x),$$
% % where each $g_j$ is concave in $u$, and the maximum of affine functions,
% % $$g(x,u) = \max_{l=1,\dots,L} (P_l(x)u + a_l(x)),$$
% % where $P_l(x)$ and $a_l(x)$ are convex functions in $x$.



% \paragraph{Box uncertainty.}
% The box uncertainty set is defined as
% $$ \uncset_{\text{box}}(\theta) = \{ u = Av+b \mid\| v\|_\infty \leq \rho, u \in S \}, $$
% where $\theta = (A,b,\rho)$, and $S$ includes additional support information such as upper and lower bounds. Note that if $A = \rho I$, $b = 0$, and $ S = \reals^m$, this is equivalent to the traditional box uncertainty set ${ \{ u \mid\| u\|_\infty \leq \rho \}}$.
% The reformulation of~\eqref{eq:canon_robust} is
% \begin{equation}
% 	\label{eq:box_max}
% 	\begin{array}{ll}
% 		\mbox{minimize} & f(z,x)\\
% 		\mbox{subject to} & H^{cT}_{(i)}s + \sigma_S(\om_i)+ b^T( H_i^{uT}s + d_i^u -\om_i) +  \rho\|A^T(H_i^{uT}s + d_i^u-\om_i)\|_1 \le d_i^c, \quad i = 1,\dots,\bar{I}.	\end{array}
% \end{equation}

% \paragraph{Ellipsoidal uncertainty.}
% The ellipsoidal uncertainty set with a general $p$-norm is defined as
% $$ \uncset(\theta) = \{u = Av+b \mid \| v\|_p \leq \rho,u \in S\},$$
% where $\theta = (A,b,\rho,p)$.
% The reformulation is
% \begin{equation}
% 	\label{eq:ellip_max}
% 	\begin{array}{ll}
% 		\mbox{minimize} & f(s)\\
% 		\mbox{subject to} & H^{cT}_{(i)}s + \sigma_S(\om_i)+ b^T( H_i^{uT}s + d_i^u -\om_i) +  \rho\|A^T(H_i^{uT}s + d_i^u-\om_i)\|_q \le d_i^c, \quad i = 1,\dots,\bar{I}.	\end{array}
% \end{equation}

% \paragraph{Budget uncertainty.}
% The budget uncertainty set requires both box and ellipsoidal constraints, and is defined as
% $$ \uncset_{\text{bud}}(\theta) = \{u = Av+b \mid \| v\|_\infty \leq \rho, \| v \|_1 \leq \Gamma, u \in S\},$$
...
% 	\begin{array}{ll}
% 		\mbox{minimize} & f(s)\\
% 		\mbox{subject to} &  H^{cT}_{(i)}s +  \sigma_S(\om_i) + b^T(H_i^{uT}s + d_i^u - \om_i) + \rho\|r_i - A^T(H_i^{uT}s + d_i^u - \om_i)\|_1 \\
...
% \paragraph{Polyhedral uncertainty.}
% The polyhedral uncertainty set is defined as
% $$\uncset(\theta) = \{ u = Av+b \mid Dv \leq d, u \in S\},$$
...
% 	\begin{array}{ll}
% 		\mbox{minimize} & f(s)\\
% 		\mbox{subject to} & H^{cT}_{(i)}s + d^T\zeta_i + b^T( H_i^{uT}s + d_i^u-\om_i) + \sigma_S(\om_i) \le d_i^c, \quad i = 1,\dots,\bar{I}\\
