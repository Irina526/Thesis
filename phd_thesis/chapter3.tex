...
Note that by the epigraph trick of Section~\ref{sec:ro_def}, these two representations are interchangable. 
...
\begin{equation}
	...
	g(u,z) \le 0,
\end{equation}
where $z \in \mathcal{Z} \subseteq \reals^n$ is an optimization variable on the compact set $\mathcal{Z}$, $u \in \supp \subseteq \reals^m$ is an uncertain parameter with convex and closed support $\supp$, governed by the distibution $\prob$, and $-g(u, z)$ is proper, convex, and lower-semicontinuous in~$u$ for all $z$, \ie, satisfying Assumption~\ref{ass:convex_l} with $J = 1$.  In Section~\ref{sec:max_concave}, we extend our results for $g$ being the maximum of of concave functions, where $J \geq 1$.
...
Bh combining the $K$ constraints in~\eqref{eq:robustopt_p}, we arrive at the form
\begin{equation}
...
Bh the same logic, in the case where $p = \infty$, the expression is equivalent to the dual of the constraint of~\eqref{eq:unif-const-p}.
...
One might note that a blanket increase in the radius regardless of the optimization problem may introduce unneccesary conservatism. This brings us to the next section, where we bring the constraint functions $g$ into our considerations. 
...
We cannot directly compare this upper bound against the change in constraint values, as at the optimal chonsen $z, X$ for each $K$, which differ due to differences in the feasible regions, the constraint value will always be near 0 for optimality. We thus compare it against the change in objective values. }
\reviewChanges{
\paragraph{Choosing $K$}
...
Bh clustering the dataset, we solve an efficient and computationally tractable formulation with limited performance degradation.
